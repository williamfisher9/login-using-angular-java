...
export class RegisterComponent {
  ...
  handleRegisterForm() {
    ...
    if (/^([a-zA-Z]){3,8}$/.test(this.registerForm.value.firstName!.trim()) === false) {
      this.errors.firstName.message.push('Incorrect first name regex');
      hasErrors = true;
    }

    if (/^([a-zA-Z]){3,8}$/.test(this.registerForm.value.lastName!.trim()) === false) {
      this.errors.lastName.message.push('Incorrect last name regex');
      hasErrors = true;
    }

    if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.registerForm.value.emailAddress!.trim()) === false) {
      this.errors.emailAddress.message.push('Incorrect email address regex');
      hasErrors = true;
    }

    if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@.#$!%*?&^])[A-Za-z\d@.#$!%*?&]{8,15}$/.test(this.registerForm.value.password!.trim()) === false) {
